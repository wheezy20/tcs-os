...
Migration `0017` creates a Django Group named **`Administration`** that bundles the three custom admissions permissions not granted automatically (`can_decide`, `can_view_health_info`, `can_send_bulk_email`) **plus** full base access within admissions: view/add/change (never delete) on every core model, and view on the read-only/audit surfaces (drafts, reference counters, the transactional-email log, the bulk-send audit).

To give a senior staff member the full admissions toolset: create their user in `/admin/`, mark them **Staff status**, and add them to the **`Administration`** group. That's the whole flow — no superuser flag needed. A non-superuser `Administration` member has a fully working admissions console; the only two things they can't do, both by design, are **delete rows** (duplicate/spam/junk cleanup stays a superuser task) and **manage other staff accounts** (creating Django users, assigning groups, resetting passwords — also superuser-only, which is why *this* onboarding step itself requires a superuser). See `02-stack-and-schema.md`'s "`Administration` = full admissions access without superuser" for the exact permission list and rationale.
