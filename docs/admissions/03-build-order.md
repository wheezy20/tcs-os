...
**Current phase: none active — Phases 1–6 DONE, plus post-Phase-6 follow-ups (a7 inquiry PDF attachment, b3 nationality typeahead, b2 async transactional email, b4 flat UTM fields on `Lead`, b5 `Administration` Group — all 2026-09-03) and Phase 6.2 (grade-band coordinator RBAC, 2026-09-04, `admissions/tests.py` at 74 tests). Next up is either a scoped Phase 6.1 (bounce/open tracking, topic-level opt-outs, funnel attribution) or Phase 7+ from `01-vision.md`. Several operational go-live items remain, and b2/b4/b5/6.2 all need deploying (migrations `0015`–`0019`) — see `docs/deployment.md`'s "Current deployment state".**
...
**A real, pre-existing gap surfaced by testing, not fixed here:** the `Administration` group (b5) grants only its three named custom permissions (`can_decide`, `can_view_health_info`, `can_send_bulk_email`) — no base Django model permissions (`view`/`change_application`, etc.). `GradeBandScopedAdmin`'s own bypass for Administration members works correctly (confirmed directly), but a **non-superuser** Administration member would still be blocked by Django's own `has_change_permission` check for lack of `change_application` and the rest. Every real Administration user so far (you) has also been a superuser, so this has never been hit in practice. Flagged for a decision, not assumed: either grant `Administration` the standard model CRUD permissions too, or document that Administration membership always implies superuser status.

**Testing:** 27 new tests (47 → **74, all green**, ~14s on in-memory SQLite): band definitions (disjoint, cover exactly the classified grades, Grade 10 unbanded); live queryset scoping across all four admins + both inlines, across both campuses; the grade-change-moves-coordinators test; `has_change_permission` object-level blocking; readonly-fields and `get_actions` per role; `move_to_document_review()` — happy path, real-`save()`-not-raw-write (assigns `application_reference`), rejects from `document_review`/terminal stages, a second call on an already-`document_review` row raises rather than double-transitioning; the admin action's partial-success reporting; `audit_staff_roles`' four scenarios. Migration `0019` proven against a fresh DB (same `create_permissions` guard as `0017`/`0019` itself).
