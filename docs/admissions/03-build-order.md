...
**Current phase: none active — Phases 1–6 DONE, plus post-Phase-6 follow-ups (a7 inquiry PDF attachment, b3 nationality typeahead, b2 async transactional email, b4 flat UTM fields on `Lead`, b5 `Administration` Group — all 2026-09-03) and Phase 6.2 (grade-band coordinator RBAC, 2026-09-04, plus a 2026-09-09 follow-up widening the `Administration` group to full non-superuser admissions access; `admissions/tests.py` at 85 tests). Next up is either a scoped Phase 6.1 (bounce/open tracking, topic-level opt-outs, funnel attribution) or Phase 7+ from `01-vision.md`. Several operational go-live items remain, and b2/b4/b5/6.2 all need deploying (migrations `0015`–`0019`) — see `docs/deployment.md`'s "Current deployment state".**
...
- Lead source tracking — **partial**: `Lead.source` distinguishes the two capture points and bulk campaigns can target `filter_lead_source`. Flat `utm_source`/`utm_medium`/`utm_campaign` fields added on `Lead` (b4, 2026-09-03, migration `0016`) — raw campaign attribution from the marketing-site URL, captured client-side and passed through in the POST body, visible/searchable in admin. Full normalised campaign/UTM attribution across the funnel (the `Activity` / `LeadSource` shape sketched in `01-vision.md`) is still open — see the two logged "someday" items under Phase 6.1 below.

## Phase 6.1 (not started, scoped) — bulk-email follow-ons
Bounce/open tracking (Resend webhooks; opens also need HTML email); per-topic / multiple mailing lists (replacing the single all-or-nothing unsubscribe flag). All deferred deliberately from Phase 6 — see that entry. Pick up if/when actually needed.

### Someday — full funnel / campaign attribution (logged, not scheduled)
Two related items, both **deferred until TCS actually runs structured paid campaigns that need spend justification**. The flat `utm_source`/`utm_medium`/`utm_campaign` fields on `Lead` (b4) are sufficient until then — this is the heavyweight version they'd grow into, not a near-term plan.

1. **Structured attribution data model** — `Campaign` (name, channel, start/end dates, budget), `LeadSource` (clean, curated channel names, mapped from the raw UTM combos that land on `Lead`), `Activity` (a timestamped per-`Lead` event log: PDF viewed, form submitted, converted to Inquiry, …), plus a real `Lead → Family` / `Application` / `Decision` attribution link so a captured lead can be followed all the way to an enrolment outcome. Supersedes the flat `utm_*` fields.
2. **Cost-per-lead / cost-per-enrolment reporting** — built on top of item 1: ties each `Campaign`'s spend/budget to the enrolled outcomes attributed to it, so marketing spend can be justified per channel. Not meaningful without the attribution model above.

## Phase 6.2 — Grade-band coordinator RBAC — DONE (2026-09-04)

Full plan (model design, queryset-filtering mechanism, the `can_view_health_info` question) proposed and approved point by point before any code, same as Phase 3/5/6. Full detail in `02-stack-and-schema.md`'s new "Phase 6.2" section; summary here.

**What shipped:**
- `StaffProfile` (migration `0018`) — `OneToOne` onto `auth.User`, one field that matters: `grade_band` (`preschool` / `primary` / `jhs` / blank = unscoped). **Authoritative** for scoping — `admissions/access.py`'s `scoped_grades_for()` reads this, never Group membership.
- `GRADE_BANDS` (in `models.py`) — derived from the existing `STUDENT_ID_CLASSIFICATION` dict (Phase 2.5) rather than a second hand-maintained grade list, so the two can never drift apart. Preschool = codes 01+02, Primary = 03, JHS = 04. A grade with no classification code (SHS, or a parent's free-text "Other" entry) belongs to no band and is visible only to `Administration`/superusers.
- Three Groups — `Preschool Coordinator`, `Primary Coordinator`, `JHS Coordinator` (migration `0019`, idempotent, same `create_permissions`-guard pattern as `0017`). Identical permission bundle on all three: `view`/`change_application`, `view`/`change_document`, `view`/`add`/`change_note`, `view_emergencycontact`, `view_student`/`view_family`/`view_guardian`, **and `can_view_health_info`** (Option B, approved — all three bands, scoped to their own band by the same mechanism as everything else). Deliberately excluded: `can_decide`, `can_send_bulk_email`, `add`/`delete_application`, any `delete_*`.
- `GradeBandScopedAdmin` / `GradeBandScopedInline` mixins (`admin.py`) — a live `get_queryset()` filter on `year_group_applied_for` (never a stored per-row assignment), applied to `ApplicationAdmin`, `StudentAdmin`, `FamilyAdmin`, `GuardianAdmin`, and the `Document`/`Note`/`EmergencyContact`/`HealthInfo` inlines. Confirmed by test that a post-submission grade edit re-resolves to the new coordinator on the next page load, no backfill.
- `Application.move_to_document_review()` — the ONE write a coordinator gets. Deliberately narrower than a plain `stage = "..."` write: only from `inquiry`/`application`, re-validated under `select_for_update()` at write time (not just when the changelist queryset was built), through the real `save()` (never a raw `.update()`). Wired to a new `ApplicationAdmin` action, `move_to_document_review`; the existing unrestricted `mark_as_application`/`mark_as_document_review`/`mark_as_enrolled` actions are hidden from coordinators via `get_actions()`. Every other Application field is readonly for a coordinator (`get_readonly_fields()`) — no open `stage` dropdown, so `rejected`/`waitlisted`/`offer`/`enrolled` are simply unreachable for them.
- `manage.py audit_staff_roles` — flags a user in a Coordinator group with no matching `grade_band` (fails closed, sees nothing), a `grade_band` set with no matching group (has scope, no permissions), or membership in `Administration` *and* a Coordinator group at once (Administration wins, scope silently ignored). Not wired into `check`/CI — meant to be run by hand after a roster change.
- `StaffProfileInline` on a re-registered `UserAdmin` — grade-band assignment lives on the same page as Group membership.

**A pre-existing gap surfaced by testing — resolved 2026-09-09 (follow-up commit):** the `Administration` group (b5) originally granted only its three named custom permissions, no base Django model permissions, so a **non-superuser** member was blocked by Django's own `has_*_permission` checks throughout the admin (never hit in practice — every Administration user so far was also a superuser). Fixed by widening migration `0017` (still unapplied, edited in place) to view+add+change on the 14 core models and view on the 4 read-only/audit surfaces; `delete_*` and all `auth.*` stay deliberately out. An audit of every registered admin + inline turned up further gaps beyond the two originally flagged (`add_guardian`/`add_student`/`add_document`/`add_emergencycontact` inline add-rows, `add_emailcampaign`, `add_capacity`, `view_applicationdraft`, `view_transactionalemail`, `view_referencecounter`). `Administration` now means full admissions access on its own — see `02-stack-and-schema.md`'s "`Administration` = full admissions access without superuser".

**Testing:** 27 new tests (47 → **74, all green**; 85 after the 2026-09-09 `Administration` follow-up): band definitions (disjoint, cover exactly the classified grades, Grade 10 unbanded); live queryset scoping across all four admins + both inlines, across both campuses; the grade-change-moves-coordinators test; `has_change_permission` object-level blocking; readonly-fields and `get_actions` per role; `move_to_document_review()` — happy path, real-`save()`-not-raw-write (assigns `application_reference`), rejects from `document_review`/terminal stages, a second call on an already-`document_review` row raises rather than double-transitioning; the admin action's partial-success reporting; `audit_staff_roles`' four scenarios. Migration `0019` proven against a fresh DB (same `create_permissions` guard as `0017`/`0019` itself).

**Not deployed this session.** Migrations `0018`+`0019` ride the same `admissions-migrate` job as `0015`–`0017`. No env-var/queue changes. Post-deploy: create the 3 coordinator users, set Staff status, add each to their Group, set their `StaffProfile.grade_band` on the same User page.
