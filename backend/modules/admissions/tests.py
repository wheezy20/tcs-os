...
_TURNSTILE_OK = mock.patch("modules.admissions.turnstile.verify_turnstile_token", return_value=None)
...
class QuickInterestEndpointTests(_PublicEndpointBase):
    ...
    def test_bad_turnstile_token_is_400(self):
        from modules.admissions import turnstile
        with mock.patch(
            "modules.admissions.turnstile.verify_turnstile_token",
...
class TransactionalEmailAsyncPathTests(_PublicEndpointBase):
    ...
    def test_submission_persists_rows_and_enqueues_without_sending_inline(self):
        with mock.patch("modules.admissions.emails.enqueue_transactional_ids") as enq:
    ...
    def test_worker_delivers_pending_rows(self):
        with mock.patch("modules.admissions.emails.enqueue_transactional_ids"):
    ...
    def test_enqueue_failure_falls_back_to_inline_send(self):
        with mock.patch(
            "modules.admissions.emails.enqueue_transactional_ids",
...
class TransactionalEmailWorkerTests(TestCase):
    ...
    def test_transient_failure_keeps_pending_and_502s(self):
        with mock.patch("modules.admissions.emails._deliver", side_effect=RuntimeError("smtp down")):
